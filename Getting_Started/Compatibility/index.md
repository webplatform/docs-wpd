...
{{Note | Extended prose discussions on compatibility may have citation and reuse issues, and so must be avoided (without training beyond this Getting Started guideline).  See the [http://docs.webplatform.org/wiki/WPD:External_Attribution attribution guidelines].}}

Background -- this starter guide assumes that you are already registered with WPD and can edit wiki markup.  See the [http://docs.webplatform.org/wiki/WPD:Getting_Started main getting started guidelines].
...
* Updating the task spreadsheet (the google doc for the Nov 2012 event at Adobe SF is/was [https://docs.google.com/spreadsheet/ccc?key=0Aoc3F7WkVTNUdGg1UnVCakExMjZBUjIxYThGdTh5X2c#gid=0 here])
...
* Use the [http://docs.webplatform.org/w/index.php?title=Special%3AWhatLinksHere&target=Template%3ACompat+Unknown&namespace= official list], and select a topic of interest.  As of Nov 2012 there are hundreds of items, so you may need to page forward for a while. (or)
...
For MDN, you can use the basic facts (version numbers, yes/no/partial on support) without attribution.  WPD has already made this arrangement.  For all others, you should follow the [http://docs.webplatform.org/wiki/WPD:External_Attribution attribution guidelines].
...
* the [http://docs.webplatform.org/w/index.php?title=html/elements/audio audio tag] which has separate rows for the various codecs covered in the HTML5 spec, since support differs significantly across vendors
