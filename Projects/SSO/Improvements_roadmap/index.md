...
** '''Create an event handler that replies with a JSON object that reads the current <tt>recoveryPayload</tt> value in localStorage. (e.g. <tt>{recoveryPayload: {packet: "...", digest: "HMAC;SHA512"}, hasSession: true}</tt>)''' 
